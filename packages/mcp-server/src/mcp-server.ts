...
class PumpFunChatServer {
  ...
  private connectToChat() {
    ...
    console.error(`[INIT] Connecting to pump.fun chat for token: ${this.token}`);
    ...
    this.client.on('connected', () => {
      ...
      console.error(`[INIT] Successfully connected to chat room for ${this.token}`);
    });
    ...
    this.client.on('disconnected', () => {
      ...
      // Disconnected from chat room - silent
    });
  ...
  async run() {
    ...
    console.error(`[INIT] Pump.fun Chat MCP server running for token: ${this.token}`);
  }
}
...
if (!token) {
  ...
  console.error(
    'Example: PUMP_FUN_TOKEN=y31hFyYbrVW4R53Zfka8WJfQpwpMLfCcAjVKAonpump pump-fun-chat-mcp'
  );
  process.exit(1);
}
